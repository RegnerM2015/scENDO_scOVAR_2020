...
#' 
#' rna.orig <- readRDS("ovar_38FE7L_scRNA_processed-original.rds")
#' rna.orig <- rna.orig[,rna.orig$seurat_clusters == "4"]
#' dead.cells <- colnames(rna.orig)
#' 
#' rna <- rna[,setdiff(colnames(rna),dead.cells)]
#' 
...
#' }
rna <- readRDS("./rna_postdoublet_SkipChecks.rds")
Idents(rna)<- "RNA_snn_res.0.7"
