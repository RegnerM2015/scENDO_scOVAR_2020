...
dir.create("./Significant_P2G_Outputs_Patient2")
SAMPLE.ID <- "EEC"
key.word.1 <- "pithelia"
key.word.2 <- "-Ciliated"
...
idxSample <- BiocGenerics::which(proj$Sample =="3571DL")
...
saveRDS(store,"./Significant_P2G_Outputs_Patient2/store_null_tests.rds")
...
pdf("./Significant_P2G_Outputs_Patient2/P2G_Correlation.pdf",width = 5,height = 3.5)
hist(p2g.df.obs$Correlation,col = "lightblue",
     main = paste0("Histogram of\n",nrow(p2g.df.obs)," P2G correlations"),xlab = "Correlation")
dev.off()

pdf("./Significant_P2G_Outputs_Patient2/P2g_Pval.pdf",width = 5,height = 3.5)
hist(p2g.df.obs$RawPVal,col="lightblue",
     main = paste0("Histogram of\n",nrow(p2g.df.obs)," P2G p-values"),xlab = "p-value")
abline(v=0.01,col = "red")
dev.off()

saveRDS(p2g.df.obs,"./Significant_P2G_Outputs_Patient2/All_P2G_Observed.rds")

################################################################################################################

p2g.df.hist <- dplyr::filter(p2g.df.obs,RawPVal <=1e-12 & Correlation >= 0.45)

pdf("./Significant_P2G_Outputs_Patient2/GenesPerPeak_histogram.pdf",width = 5,height = 3.5)
hist(table(p2g.df.hist$idxRNA),main="Distribution of genes per peaks")
dev.off()

pdf("./Significant_P2G_Outputs_Patient2/PeaksPerGene_histogram.pdf",width = 5,height = 3.5)
...
cols <- cols[c(3,6,9,16,17,19,20,21,22,25,26)]
...
pdf("./Significant_P2G_Outputs_Patient2/Peak2Gene_Heatmap_Legend.pdf",width = 8,height = 12)
...
saveRDS(p2g.df.sub.plot,"./Significant_P2G_Outputs_Patient2/Distal_P2Gs_Kmeans.rds")

pdf("./Significant_P2G_Outputs_Patient2/GenesPerPeak_histogram-distal.pdf",width = 5,height = 3.5)
hist(table(p2g.df.sub.plot$idxRNA),main="Distribution of genes per distal peaks")
dev.off()

pdf("./Significant_P2G_Outputs_Patient2/PeaksPerGene_histogram-distal.pdf",width = 5,height = 3.5)
