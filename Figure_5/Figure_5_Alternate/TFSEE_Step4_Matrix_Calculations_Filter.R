...
drug.scores <- drug.scores[-17,]# Remove duplicate DDIT3
...
# tfs <- rownames(expr.mat.scatter)
# 
# tfsee <- tfsee.scatter[,colnames(tfsee.scatter) %in% tfs]
# 
...
# 
# expr.serous <- expr.mat.scatter[,c(6,11)]
...
# 
# expr.endo <- as.data.frame(expr.mat.scatter[,5])
...
#   scale_color_manual(values = c("gray60","black"))+ggsave("TFSEE_FC_serous_ov_v_endo_ov.pdf",width =5,height = 4)
# 
# 
# 
# 
...
# 
# 
# # Scatter plot of log2FC TFSEE score versus log2FC TF expression (Endo Endo v. Endo OV)
# 
# # Subset data to the most robustly expressed TFs across pseudobulk clusters
# tfs <- rownames(expr.mat.scatter)
# 
...
# 
# # Build TFSEE fold change vector
# tfsee.endo <- t(tfsee[c(1:2),])
# tfsee.endo <- as.data.frame(rowMeans(tfsee.endo))#Take mean TFSEE value between two serous clusters
# colnames(tfsee.endo) <- "ENDO"
# 
# tfsee.ovar <- tfsee[5,]
# tfsee.ovar <- as.data.frame(tfsee.ovar)
# colnames(tfsee.ovar) <- "OVAR"
# 
# length(which(rownames(tfsee.endo) == rownames(tfsee.ovar)))
# 
# FC.tfsee <- log2(tfsee.endo$ENDO/tfsee.ovar$OVAR)
# hist(FC.tfsee)
# 
# # Build Expression fold change vector 
# expr.endo <- expr.mat.scatter[,c(1:2)]
# expr.endo <- as.data.frame(rowMeans(expr.endo))#Take mean TFSEE value between two serous clusters
# colnames(expr.endo) <- "ENDO"
# 
# expr.ovar <- expr.mat.scatter[,5]
# expr.ovar <- as.data.frame(expr.ovar)
# colnames(expr.ovar) <- "OVAR"
# 
# length(which(rownames(expr.serous) == rownames(expr.endo)))
# 
# FC.expr <- log2(expr.endo$ENDO/expr.ovar$OVAR)
# hist(FC.expr)
# 
# 
# length(which(rownames(tfsee.serous) == rownames(expr.endo)))
...
# 
# total.endo <- dplyr::filter(total,sum > 0)
# total.ovar <- dplyr::filter(total,sum < 0)
# 
# drug.endo <- intersect(rownames(total.endo),drug.scores.sub$TF)
# drug.ovar <- intersect(rownames(total.ovar),drug.scores.sub$TF)
# 
# total.endo <- total.endo[rownames(total.serous) %in% drug.serous,]
# total.ovar <- total.ovar[rownames(total.ovar) %in% drug.endo,]
# 
# 
# 
# # Make Scatter plot for FC TFSEE versus FC Expression
...
#   ylab("log2FC(TF Expression)")+theme_bw()+
#   xlab("log2FC(Raw TFSEE Score)")+
#   scale_color_manual(values = c("gray60","black"))+ggsave("TFSEE_FC_Endo_Endo_v_Endo_OV.pdf",width =5,height = 4)
# 
# 
# 
# 
...
# 
# 
# # Scatter plot of log2FC TFSEE score versus log2FC TF expression (SEROUS ENDO v. ENDO ENDO)
# 
# 
# # Scatter plot of log2FC TFSEE score versus log2FC TF expression (Serous Ovarian v. Endometrioid Ovarian)
# 
...
# # Build Expression fold change vector 
# 
# expr.serous <- expr.mat.scatter[,c(3:4)]
# expr.serous <- as.data.frame(rowMeans(expr.serous))#Take mean TFSEE value between two serous clusters
# colnames(expr.serous) <- "Serous"
# 
# expr.endo <- expr.mat.scatter[,c(1:2)]
...
#   ylab("log2FC(TF Expression)")+theme_bw()+
#   xlab("log2FC(Raw TFSEE Score)")+
#   scale_color_manual(values = c("gray60","black"))+ggsave("TFSEE_FC_serous_endo_v_endo_endo.pdf",width =5,height = 4)
# 
# 
# 
# 
...
# # Build Expression fold change vector 
# expr.mat
# expr.serous <- expr.mat[,c(6,11)]
# expr.serous <- as.data.frame(rowMeans(expr.serous))#Take mean TFSEE value between two serous clusters
# colnames(expr.serous) <- "Serous"
# 
# expr.endo <- as.data.frame(expr.mat[,5])
# colnames(expr.endo) <- "Endometrioid"
# 
# length(which(rownames(expr.serous) == rownames(expr.endo)))
# 
...
# 
# 
# # Scatter plot of log2FC TFSEE score versus log2FC TF expression (Metastatic endometrial v. Endometrioid Endo)
# 
# # Subset data to the most robustly expressed TFs across pseudobulk clusters
# tfs <- rownames(expr.mat)
# 
...
# length(which(rownames(tfsee.serous) == rownames(tfsee.endo)))
# 
# FC.tfsee <- log2(tfsee.serous[,1]/tfsee.endo[,1])
# hist(FC.tfsee)
# 
# # Build Expression fold change vector 
# expr.serous <- expr.mat[,c(3:4)]
# expr.serous <- as.data.frame(rowMeans(expr.serous))#Take mean TFSEE value between two serous clusters
# colnames(expr.serous) <- "serous"
# 
# expr.endo <- expr.mat[,c(1:2)]
# expr.endo <- as.data.frame(rowMeans(expr.endo))#Take mean TFSEE value between two serous clusters
# colnames(expr.endo) <- "Endometrioid"
# 
# length(which(rownames(expr.serous) == rownames(expr.endo)))
# 
# FC.expr <- log2(expr.serous[,1]/expr.endo[,1])
# hist(FC.expr)
# 
# 
# length(which(rownames(tfsee.serous) == rownames(expr.endo)))
...
# 
# 
# # Scatter plot of log2FC TFSEE score versus log2FC TF expression (GIST v. HGSOC)
# 
# # Subset data to the most robustly expressed TFs across pseudobulk clusters
# tfs <- rownames(expr.mat)
# 
...
# 
# # Build TFSEE fold change vector
# tfsee.serous <- as.data.frame(t(tfsee[c(9,10),]))
# tfsee.serous <- as.data.frame(rowMeans(tfsee.serous))#Take mean TFSEE value between two serous clusters
# colnames(tfsee.serous) <- "GIST"
# 
# tfsee.endo <- as.data.frame(t(tfsee[c(6,11),]))
# tfsee.endo <- as.data.frame(rowMeans(tfsee.endo))#Take mean TFSEE value between two serous clusters
# colnames(tfsee.endo) <- "HGSOC"
# 
# length(which(rownames(tfsee.serous) == rownames(tfsee.endo)))
# 
# FC.tfsee <- log2(tfsee.serous[,1]/tfsee.endo[,1])
...
# 
# # Build Expression fold change vector 
# expr.serous <- as.data.frame(expr.mat[,c(9,10)])
# expr.serous <- as.data.frame(rowMeans(expr.serous))#Take mean TFSEE value between two serous clusters
# colnames(expr.serous) <- "GIST"
# 
# expr.endo <- as.data.frame(expr.mat[,c(6,11)])
# expr.endo <- as.data.frame(rowMeans(expr.endo))#Take mean TFSEE value between two serous clusters
# colnames(expr.endo) <- "HGSOC"
# 
# length(which(rownames(expr.serous) == rownames(expr.endo)))
# 
# FC.expr <- log2(expr.serous[,1]/expr.endo[,1])
...
#   ylab("log2FC(TF Expression)")+theme_bw()+
#   xlab("log2FC(Raw TFSEE Score)")+
#   scale_color_manual(values = c("gray60","black"))+ggsave("TFSEE_FC_GIST_v_HGSOC.pdf",width =5,height = 4)
# 
# 
# 
# 
