...
tfsee <- t(tfsee.t)
write.csv(data.frame(colnames(tfsee)),"TFs_for_canSAR-intermediate.csv")
...
drug.scores <- read.csv("./cpat-results-intermediate.csv")
...
drug.scores <- drug.scores[-10,]# Remove duplicate DDIT3
...
p1<-ggplot(total,aes(x=rank,y = difference.tfsee))+
  ...
  geom_hline(yintercept = -0.25,linetype="dashed")+
  ggsave("TFSEE_Sublcone_Comparison-intermediate.pdf",width =5,height = 4)
...
p2<-ggplot(total,aes(x=rank,y = difference.tfsee))+
  ...
  geom_hline(yintercept = 0.25,linetype="dashed")+
  geom_hline(yintercept = -0.25,linetype="dashed")+
  ggsave("TFSEE_Carcinosarcoma-intermediate.pdf",width =5,height = 4)
...
p3<-ggplot(total,aes(x=rank,y = difference.tfsee))+
  ...
  geom_hline(yintercept = 0.25,linetype="dashed")+
  geom_hline(yintercept = -0.25,linetype="dashed")+
  ggsave("TFSEE_SerousVEndometrioid_Endometrial-intermediate.pdf",width =5,height = 4)
...
p4<-ggplot(total,aes(x=rank,y = difference.tfsee))+
  ...
  geom_hline(yintercept = 0.25,linetype="dashed")+
  geom_hline(yintercept = -0.25,linetype="dashed")+
  ggsave("TFSEE_SerousVEndometrioid_Ovarian-intermediate.pdf",width =5,height = 4)
...
p5<-ggplot(total,aes(x=rank,y = difference.tfsee))+
  ...
  geom_hline(yintercept = 0.25,linetype="dashed")+
  geom_hline(yintercept = -0.25,linetype="dashed")+
  ggsave("TFSEE_GISTvHGSOC-intermediate.pdf",width =5,height = 4)


CombinePlots(list(p4,p3,p5),ncol=1)+ggsave("Freq_Distribtions_Suppl-intermediate.pdf",
                                           width=5,height = 12)

CombinePlots(list(p1,p2),ncol=2)+ggsave("Freq_Distribtions_Main-intermediate.pdf",
                                           width=10,height =4)
...
pdf(paste0("TFSEE_celltype_scaled-intermediate.pdf"), width = 7, height =11)
...
writeLines(capture.output(sessionInfo()), "sessionInfo_TFSEE_Plotting-intermediate.txt")
